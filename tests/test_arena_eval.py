...
def test_arena_eval_returns_correct_keys(mock_ppo):
...
@patch("rl_framework.training.arena_eval.PPO")
def test_arena_eval_win_rates_sum_to_lte_one(mock_ppo):
    mock_ppo.load.return_value = _mock_policy()
    result = run_arena_eval(
...
@patch("rl_framework.training.arena_eval.PPO")
def test_arena_eval_role_swap_doubles_episode_count(mock_ppo):
    mock_ppo.load.return_value = _mock_policy()
    result = run_arena_eval(
...
@patch("rl_framework.training.arena_eval.PPO")
def test_arena_eval_random_opponent_no_load(mock_ppo):
