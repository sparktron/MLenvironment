...
class OrganismArenaParallelEnv(ParallelEnv):
    ...
    def __init__(self, cfg: dict[str, Any], render_mode: str | None = None):
        ...
        self._damage_scale: float = 1.0
        self._fig = None
        self._ax = None

    def observation_space(self, agent: str):
        # vel_x, vel_y, health, rel_opp_x, rel_opp_y, opp_health, cooldown_frac
    ...
    def _obs(self, agent: str) -> np.ndarray:
        ...
        vel = me["pos"] - prev
        self._prev_positions[agent] = me["pos"].copy()
        rel = other["pos"] - me["pos"]
        dist_to_opp = float(np.linalg.norm(rel))
        if dist_to_opp > self.rules.sensing_radius:
            rel_x, rel_y, opp_health = 0.0, 0.0, 0.0
        else:
            rel_x, rel_y, opp_health = (
                float(rel[0]),
                float(rel[1]),
                float(other["health"]),
            )
        cooldown_frac = float(me["cooldown"]) / max(1, self.rules.cooldown_steps)
        return np.array(
            [vel[0], vel[1], me["health"], rel_x, rel_y, opp_health, cooldown_frac],
            dtype=np.float32,
        )
    ...
    def step(self, actions: dict[str, np.ndarray]):
        ...
        for attacker in list(self.agents):
            ...
            if self.rules.attack_falloff == "linear":
                falloff = max(0.0, 1.0 - dist / self.rules.attack_range)
            else:
                falloff = float(dist <= self.rules.attack_range)
            damage = (
                self.rules.damage
                * falloff
                * self.state[attacker]["size"]
                * self._damage_scale
            )
            if damage > 0:
        ...
        if any(terminations.values()):
            for agent in active_agents:
                if terminations[agent]:
                    winner = "agent_1" if agent == "agent_0" else "agent_0"
                    outcome = {
                        "winner": winner if winner in active_agents else None,
                        "loser": agent,
                        "outcome": "ko",
                        "step": self.step_count,
                    }
                    infos[agent]["episode_outcome"] = outcome
                    if winner in infos:
                        infos[winner]["episode_outcome"] = outcome
        elif all(truncations.values()):
            for agent in active_agents:
                infos[agent]["episode_outcome"] = {
                    "winner": None,
                    "outcome": "timeout",
                    "step": self.step_count,
                }
