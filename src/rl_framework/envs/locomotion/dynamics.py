...
ACTION_SCALE = np.array(
    [1.2, 1.2, 0.5, 1.2, 1.2, 0.5,   # legs (hip/knee/ankle × 2)
     1.2, 1.0, 1.2, 1.0],            # arms (shoulder/elbow × 2)
    dtype=np.float32,
)
...
class WalkerDynamics:
    ...
    def apply_action(self, body_id: int, action: np.ndarray, physicsClientId: int = 0) -> None:
        ...
        forces = (_TORQUE_CAPS * self.torque_scale).tolist()

        if self.control_mode == "pd":
            targets = np.clip(REST_POSE + a * ACTION_SCALE, _LOWS, _HIGHS).tolist()
            p.setJointMotorControlArray(
                body_id, JOINT_INDICES,
                controlMode=p.POSITION_CONTROL,
                targetPositions=targets,
                positionGains=[self.position_gain] * NUM_JOINTS,
                velocityGains=[self.velocity_gain] * NUM_JOINTS,
                forces=forces,
