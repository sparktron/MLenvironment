...
class WalkerDynamics:
    ...
    def apply_action(self, body_id: int, action: np.ndarray, physicsClientId: int = 0) -> None:
        ...
        p.applyExternalForce(body_id, -1, [fx, fy, 0.0], [0.0, 0.0, 0.0], p.WORLD_FRAME, physicsClientId=physicsClientId)
        p.applyExternalTorque(body_id, -1, [0.0, 0.0, torque_z], p.WORLD_FRAME, physicsClientId=physicsClientId)
